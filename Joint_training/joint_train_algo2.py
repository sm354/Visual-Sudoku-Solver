...
classifier_truenoisy = torch.load(args.pretr_classifier, map_location=device)
classifier_truenoisy.eval()
...
for epoch in range(num_epochs):
    ...
    if(epoch%25 == 0):
        #deepcopy the classifier into classifier_truenoisy
        classifier_truenoisy = copy.deepcopy(classifier)
        classifier_truenoisy.eval()
    ...
    for batch_id, (X_img, Y_img) in enumerate(train_data_loader):
        ...
        with torch.no_grad():
            X_truenoisy = classifier_truenoisy(X_img.reshape(-1, 1, 28, 28).float()).reshape(X_img.shape[0], sudoku_cells*sudoku_cells, -1).argmax(dim=2)
            Y_truenoisy = classifier_truenoisy(Y_img.reshape(-1, 1, 28, 28).float()).reshape(X_img.shape[0], sudoku_cells*sudoku_cells, -1).argmax(dim=2)
        ...
        if(args.loss_reg == 'yes' and epoch%25<5): #epoch condition removes the regularization once the epoch has been reached
